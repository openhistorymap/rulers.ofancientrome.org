...
Q_HUMAN = "Q5"
# Positions whose start/end qualifiers we treat as a "reign"
RULING_POSITIONS = {
...
def reign_years(entity):
    """Min start / max end (signed years) across P39 positions that are in
    RULING_POSITIONS and carry start/end qualifiers. Returns (from, to) or
    (None, None)."""
    starts, ends = [], []
    for st in claims(entity, P_POSITION_HELD):
        pos = _dv(_mainsnak(st))
        if not (isinstance(pos, dict) and pos.get("id") in RULING_POSITIONS):
            continue
